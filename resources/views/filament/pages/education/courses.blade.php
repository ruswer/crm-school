<x-filament::page>
    ...
    <div class="flex flex-col lg:flex-row lg:items-start gap-4">
        {{-- Chap panel (Kurs qo'shish) --}}
        <div class="w-full lg:w-[30%] bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex-shrink-0">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4 pb-4 border-b dark:border-gray-700">Kurs qo'shish</h2>
            {{-- Forma --}}
            <form wire:submit.prevent="createCourse" class="space-y-6"> {{-- space-y-6 ga o'zgartirildi --}}
                {{ $this->createForm }} {{-- Filament formasini chiqarish --}}

                {{-- Saqlash tugmasi --}}
                <div class="flex justify-end pt-4">
                    <x-filament::button type="submit" wire:loading.attr="disabled">
                         <svg wire:loading wire:target="createCourse" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <table class="min-w-full text-left text-sm whitespace-nowrap">
                    <thead class="uppercase tracking-wider border-b-2 dark:border-neutral-600 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="sortBy('status')">
                                <div class="flex items-center gap-x-1">
                                    <span>Holati</span>
                                    <x-heroicon-s-chevron-up-down class="w-4 h-4 {{ $sortField === 'status' ? '' : 'text-gray-400' }}" />
                                </div>
                            </th>
                    ...
                    <tbody>
                        @forelse($this->courses as $course)
                            <tr class="border-b dark:border-neutral-600 hover:bg-gray-50 dark:hover:bg-gray-700" wire:key="course-{{ $course->id }}">
                                <td class="px-6 py-4 font-medium">{{ $course->name }}</td>
                                <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ Str::limit($course->description, 50) }}</td> {{-- Tavsifni qisqartirish --}}
                                <td class="px-6 py-4">
                                    {{-- Statusni ko'rsatish --}}
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        {{-- Tahrirlash tugmasi --}}
                                        <button type="button" wire:click="editCourse({{ $course->id }})" title="Tahrirlash"
                                        ...
                                        {{-- O'chirish tugmasi --}}
                                        <button type="button" wire:click="openDeleteModal({{ $course->id }})"
    ...
    @if($showEditModal && $editingCourse)
        <div x-data="{ show: @entangle('showEditModal') }"
             ...
             aria-labelledby="edit-course-modal-title" role="dialog" aria-modal="true" style="display: none;">
            ...
            <div x-show="show" x-transition:enter="ease-out duration-300"
                 ...
                 <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                     <div class="sm:flex sm:items-start">
                         <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                             <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="edit-course-modal-title">
                                 Kursni Tahrirlash: {{ $editingCourse->name }}
                             </h3>
                             <div class="mt-4">
                 ...
                 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                     <button type="button" wire:click="updateCourse" wire:loading.attr="disabled" wire:target="updateCourse"
                         ...
                         <svg wire:loading wire:target="updateCourse" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                         <span wire:loading.remove wire:target="updateCourse">O'zgarishlarni Saqlash</span>
                         <span wire:loading wire:target="updateCourse">Saqlanmoqda...</span>
                     </button>
    ...
    @if($showDeleteModal && $deletingCourseId)
        <div x-data="{ show: @entangle('showDeleteModal') }"
             x-show="show"
             x-on:keydown.escape.window="show = false"
             class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto"
             aria-labelledby="delete-course-modal-title" role="dialog" aria-modal="true" style="display: none;">
            <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                 x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                 x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                 class="fixed inset-0 bg-gray-500/75 transition-opacity" @click="show = false" aria-hidden="true"></div>
            ...
            <div x-show="show" x-transition:enter="ease-out duration-300"
                 ...
                 <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                     <div class="sm:flex sm:items-start">
                         ...
                         <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                             <h3 class="text-lg font-semibold leading-6 text-gray-900 dark:text-gray-100" id="delete-course-modal-title">
                                 Kursni o'chirishni tasdiqlang
                             </h3>
                             <div class="mt-2">
                                 <p class="text-sm text-gray-500 dark:text-gray-400">
                                     Haqiqatan ham "<strong>{{ $deletingCourseName }}</strong>" nomli kursni o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
                                 </p>
                             </div>
                         </div>
                     </div>
                 ...
                 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                     <button type="button" wire:click="confirmDeleteCourse" wire:loading.attr="disabled" wire:target="confirmDeleteCourse"
                         ...
                         <svg wire:loading wire:target="confirmDeleteCourse" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                         Ha, o'chirish
