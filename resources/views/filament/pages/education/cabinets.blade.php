<x-filament::page>
    ...
    <div class="flex flex-col lg:flex-row lg:items-start gap-4">
        {{-- Chap panel (Kabinet qo'shish) --}}
        <div class="w-full lg:w-[30%] bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex-shrink-0">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4 pb-4 border-b dark:border-gray-700">Kabinet qo'shish</h2>
            {{-- Forma --}}
            <form wire:submit.prevent="createCabinet" class="space-y-6"> {{-- space-y-6 ga o'zgartirildi --}}
                ...
                <div class="flex justify-end pt-4">
                    <x-filament::button type="submit" wire:loading.attr="disabled">
                         {{-- Loading spinner --}}
                         <svg wire:loading wire:target="createCabinet" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        {{-- O'ng panel (Kabinetlar ro'yxati) --}}
        <div class="w-full lg:w-[70%] bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4 pb-4 border-b dark:border-gray-700">Kabinetlar ro'yxati</h2>
            ...
            <div class="mb-4">
                 ...
                 <div class="relative">
                     ...
                     <input type="text" id="table-search" wire:model.live.debounce.500ms="search"
                            ...
                            placeholder="Kabinet nomi bo'yicha...">
                 </div>
            </div>
                ...
                <table class="min-w-full text-left text-sm whitespace-nowrap">
                    <thead class="uppercase tracking-wider border-b-2 dark:border-neutral-600 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">
                        <tr>
                            ...
                            {{-- <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="sortBy('capacity')">...</th> --}}
                            {{-- <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="sortBy('branch_id')">...</th> --}}
                            <th scope="col" class="px-6 py-3 text-right">Amallar</th>
                        </tr>
                    </thead>
                    <tbody>
                        {{-- Livewire orqali ma'lumotlarni chiqarish --}}
                        @forelse($this->cabinets as $cabinet)
                            <tr class="border-b dark:border-neutral-600 hover:bg-gray-50 dark:hover:bg-gray-700" wire:key="cabinet-{{ $cabinet->id }}">
                                <td class="px-6 py-4 font-medium">{{ $cabinet->name }}</td>
                                {{-- Kerak bo'lsa boshqa ustunlar --}}
                                {{-- <td class="px-6 py-4">{{ $cabinet->capacity }}</td> --}}
                                {{-- <td class="px-6 py-4">{{ $cabinet->branch->name ?? 'N/A' }}</td> --}}
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        {{-- Tahrirlash tugmasi --}}
                                        <button type="button" wire:click="editCabinet({{ $cabinet->id }})" title="Tahrirlash"
                                        ...
                                        {{-- O'chirish tugmasi --}}
                                        <button type="button" wire:click="openDeleteModal({{ $cabinet->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400"> {{-- Colspan jadvaldagi ustunlar soniga mos bo'lishi kerak --}}
                                    Kabinetlar topilmadi.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @if($showEditModal && $editingCabinet)
        <div x-data="{ show: @entangle('showEditModal') }"
             x-show="show"
             x-on:keydown.escape.window="show = false"
             class="fixed inset-0 z-40 flex items-center justify-center overflow-y-auto"
             aria-labelledby="edit-cabinet-modal-title" role="dialog" aria-modal="true" style="display: none;">
            {{-- Fon --}}
            ...
            {{-- Modal paneli --}}
            <form wire:submit.prevent="updateCabinet">
                <div x-show="show" x-transition:enter="ease-out duration-300"
                     ...
                     <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                         <div class="sm:flex sm:items-start">
                             <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                 <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="edit-cabinet-modal-title">
                                     Kabinetni Tahrirlash: {{ $editingCabinet->name }}
                                 </h3>
                     ...
                     <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                         ...
                         <button type="submit" wire:loading.attr="disabled" wire:target="updateCabinet"
                             ...
                             <svg wire:loading wire:target="updateCabinet" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                             <span wire:loading.remove wire:target="updateCabinet">O'zgarishlarni Saqlash</span>
                             <span wire:loading wire:target="updateCabinet">Saqlanmoqda...</span>
                         </button>
    ...
    @if($showDeleteModal && $deletingCabinetId)
        <div x-data="{ show: @entangle('showDeleteModal') }"
             x-show="show"
             x-on:keydown.escape.window="show = false"
             class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto"
             aria-labelledby="delete-cabinet-modal-title" role="dialog" aria-modal="true" style="display: none;">
            {{-- Fon --}}
            <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                 x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                 x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            ...
            <div x-show="show" x-transition:enter="ease-out duration-300"
                 ...
                 <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                     <div class="sm:flex sm:items-start">
                         ...
                         <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                             <h3 class="text-lg font-semibold leading-6 text-gray-900 dark:text-gray-100" id="delete-cabinet-modal-title">
                                 Kabinetni o'chirishni tasdiqlang
                             </h3>
                             <div class="mt-2">
                                 <p class="text-sm text-gray-500 dark:text-gray-400">
                                     Haqiqatan ham "<strong>{{ $deletingCabinetName }}</strong>" nomli kabinetni o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
                                 </p>
                             </div>
                         </div>
                     </div>
                 ...
                 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                     ...
                     <button type="button" wire:click="confirmDeleteCabinet" wire:loading.attr="disabled" wire:target="confirmDeleteCabinet"
                         ...
                         <svg wire:loading wire:target="confirmDeleteCabinet" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                         Ha, o'chirish
