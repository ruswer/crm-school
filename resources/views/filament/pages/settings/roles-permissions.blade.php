<x-filament::page>
    <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="w-full lg:w-[30%] bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 h-fit">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4 pb-4 border-b dark:border-gray-700">Rol qo'shish</h2>
            <form wire:submit.prevent="createRole" class="space-y-4">
                ...
                <div class="flex justify-end pt-4">
                    <button type="submit"
                        ...
                        <svg wire:loading wire:target="createRole" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div class="overflow-x-auto">
                ...
                <table class="min-w-full text-left text-sm whitespace-nowrap divide-y divide-gray-200 dark:divide-white/10">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                        ...
                        <tr wire:key="role-{{ $role->id }}" class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-6 py-4 text-gray-900 dark:text-white">{{ $role->name }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <!-- Ruxsatlarni belgilash -->
                                    <button type="button" wire:click="assignPermissions({{ $role->id }})"
                                            title="Ruxsatlarni belgilash"
                                    ...
                                    <!-- Tahrirlash -->
                                    <button type="button" wire:click="openModal('edit', {{ $role->id }})"
                                            title="Tahrirlash"
                                    ...
                                    <!-- O'chirish -->
                                    <button type="button" wire:click="openModal('delete', {{ $role->id }})"
                                            title="O'chirish"
            ...
            >
                <div class="flex justify-between items-center pb-3 border-b dark:border-gray-700">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-100">
                        <span x-text="$wire.modalType === 'edit' ? 'Rolni tahrirlash' : 'Rolni o\'chirish'"></span>
                    </h3>
                ...
                <div class="mt-4">
                    <div x-show="$wire.modalType === 'edit'">
                        <form wire:submit.prevent="updateRole" class="space-y-4">
                            <div>
                                <label for="editName" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Rol nomi</label>
                            ...
                            <!-- Ruxsatlar -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ruxsatlar</label>
                                <div class="space-y-2 max-h-[50vh] overflow-y-auto">
                                    @foreach($permissions as $group => $items)
                                        <div class="border-b dark:border-gray-700 pb-2">
                                            <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ \Illuminate\Support\Str::title($group) }}</h4>
                                            @foreach($items as $permission)
                                                <div class="flex items-center">
                                                    <input wire:model.defer="selectedPermissions.{{ $permission['id'] }}"
                                                           type="checkbox"
                    ...
                    <div x-show="$wire.modalType === 'delete'" class="space-y-4">
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Rolni o'chirish</h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    Rostdan ham "<span class="font-medium">{{ $roleToDeleteName }}</span>" rolini o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
                                </p>
                            </div>
                        </div>
                        <div class="mt-6 flex justify-end gap-x-3">
                            ...
                            <button type="button" wire:click="deleteRole" wire:loading.attr="disabled" wire:target="deleteRole" class="inline-flex justify-center items-center rounded-lg border border-transparent bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50">
                                <svg wire:loading wire:target="deleteRole" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                O'chirish
